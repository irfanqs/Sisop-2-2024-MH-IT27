...
### Penjelasan
- Berikut merupakan isi file dari **management.c**
```c
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
```
Fungsi run_exec: Ini adalah fungsi utilitas yang digunakan untuk menjalankan perintah eksekusi eksternal (dalam hal ini, menggunakan fork() dan execv()). Ini digunakan untuk menjalankan perintah wget dan unzip untuk mendownload dan mengekstrak file.
```c
void run_exec(char path[], char *argv[]) {
    pid_t child_id;
    int status;
...
```
Fungsi deskripsiNamafile: Fungsi ini mengimplementasikan algoritma enkripsi Caesar dengan pergeseran 7 karakter untuk mendekripsi nama file. Ini berfungsi untuk mendekripsi nama file yang telah dienkripsi sebelumnya.
```c
 deskripsiNamafile(filename);
...
```
Fungsi jenisKode: Fungsi ini digunakan untuk mengecek apakah sebuah string mengandung kode tertentu.
```c
bool jenisKode(char *filename, char *code) {
    return strstr(filename, code) != NULL;
}
```
Fungsi processFiles: Fungsi ini melakukan deskripsi file
```c
void processFiles(char *mode) {
    struct dirent *de;
    DIR *dr = opendir(".");
